...
for name,limit in {'LIVE_STATE.json':8192,'TASK_STACK.json':8192,'PROJECT_MAP.md':18432,'OPERATING_RULES.md':14336,'READ_FIRST.md':8192}.items():
    size=(H/name).stat().st_size
    if size>limit: fail('budget_exceeded=%s:%d>%d'%(name,size,limit))

live=load_json('LIVE_STATE.json'); stack=load_json('TASK_STACK.json')
...
op=live['operation']; status=op['status']
if status not in ('IDLE','IN_PROGRESS','STALE'): fail('operation_status')
if status=='IN_PROGRESS':
    for key in ('operation_id','expected_units','completed_units','resume_rule','output_root'):
        if key not in op: fail('in_progress_missing_'+key)
    if sorted(op['expected_units'])!=['A_local_max','B_local_rmse','C_p90_blockmax']: fail('expected_units')
...
if exp['ort_stage1']!='COMPLETE_ALL_PASS': fail('ort_stage')
if exp['qnn_stage1'] not in ('NOT_STARTED','IN_PROGRESS','COMPLETE'): fail('qnn_stage')
if status=='IN_PROGRESS' and exp['qnn_stage1']!='IN_PROGRESS': fail('operation_stage_mismatch')
if status=='IDLE' and exp['qnn_stage1']=='IN_PROGRESS': fail('idle_stage_mismatch')
ids=[x['id'] for x in exp['candidates']]
...
if len(sys.argv)==3 and sys.argv[1]=='--local-root':
    ...
    checks=[(live['current_host_best']['model_path'],live['current_host_best']['sha256'])]
    for obj in ('candidate_manifest','prereg','proxy_rank','stage1_ort_semantic'):
        checks.append((exp[obj]['path'],exp[obj]['sha256']))
    for cand in exp['candidates']: checks.append((cand['path'],cand['sha256']))
    for rel,expected in checks:
        p=root/rel
        if not p.is_file(): fail('missing_artifact='+rel)
        if sha256(p)!=expected: fail('artifact_sha='+rel)
    completed=set(op.get('completed_units',[])) if status=='IN_PROGRESS' else set()
    for cid in ids:
        q=root/'k8_partial_guided_tree_family'/'stage1'/('qnn_'+cid)
        done=(q/'output'/'Result_14'/'spec_real.raw').is_file() and (q/'output'/'Result_14'/'spec_imag.raw').is_file()
        if exp['qnn_stage1']=='NOT_STARTED' and done: fail('stale_qnn_results_exist='+cid)
        if status=='IN_PROGRESS':
            if cid in completed and not done: fail('completed_unit_missing_output='+cid)
            if cid not in completed and done: fail('unpublished_completed_unit='+cid)
