...
# disk science-state checks: if QNN outputs exist, handoff is stale and must be updated before further science
...
# referenced local artifact hashes
for cand in c['candidates']:
    p=ROOT/cand['model_path']
    if not p.is_file(): fail('missing_candidate='+cand['id'])
    if sha(p)!=cand['sha256']: fail('candidate_sha='+cand['id'])
for rel,expected in [(s['active_experiment']['stage1_ort_semantic_path'],s['active_experiment']['stage1_ort_semantic_sha256']),(s['active_experiment']['prereg_path'],s['active_experiment']['prereg_sha256']),(s['active_experiment']['proxy_rank_path'],s['active_experiment']['proxy_rank_sha256'])]:
    p=ROOT/rel
    if not p.is_file() or sha(p)!=expected: fail('artifact='+rel)
# handoff internal SHA references
g=s['global_context']
for filekey,shakey in [('REPOSITORY_MAP.md','repository_map_sha256'),('PROJECT_GLOBAL_CONTEXT.md','project_context_sha256'),('ROADMAP_AND_DECISION_TREE.md','roadmap_sha256'),('NEXT_HORIZON.json','next_horizon_sha256'),('ANTI_RABBIT_HOLE.md','anti_rabbit_hole_sha256')]:
    if sha(H/filekey)!=g[shakey]: fail('handoff_sha='+filekey)
if sha(H/'RECOVERY_ANCHORS.md')!=s['recovery']['anchors_sha256']: fail('recovery_sha')
if sha(H/'CANDIDATE_MANIFEST.json')!=s['candidate_public_manifest']['sha256']: fail('candidate_manifest_sha')
if sha(H/'INVALID_AND_CLOSED_SUMMARY.md')!=s['invalid_closed_summary']['sha256']: fail('invalid_summary_sha')
